...
class TRADE(nn.Module):
	def __init__(self, hidden_size, lang, path, task, lr, dropout, slots, gating_dict, nb_train_vocab=0):
		...
		self.decoder = GeneratorATT(self.lang, self.encoder.embedding, self.lang.n_words, hidden_size, self.dropout, self.slots, self.nb_gate) 
...
class EncoderATT(nn.Module):
	def __init__(self, vocab_size, hidden_size, dropout, n_layers=1):
		...
		self.MH = nn.MultiheadAttention(hidden_size,1,dropout)
...
class GeneratorATT(nn.Module):
	def __init__(self, lang, shared_emb, vocab_size, hidden_size, dropout, slots, nb_gate):
		...
		self.slots = slots
		self.MH = nn.MultiheadAttention(hidden_size,1,dropout)
		self.W_gate = nn.Linear(3*hidden_size, nb_gate)
	...
	def forward(self, batch_size,   encoded_hidden,encoded_outputs, encoded_lens, story, max_res_len, target_batches, use_teacher_forcing, slot_temp):
		...
		for slot in slot_temp:
			...
			decoder_input,_ = self.MH(decoder_input,decoder_input,decoder_input)
			context = []
			...
			all_gate_outputs[counter] = self.W_gate(context)
			counter += 1
