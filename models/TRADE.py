...
class TRADE(nn.Module):
	def __init__(self, hidden_size, lang, path, task, lr, dropout, slots, gating_dict, nb_train_vocab=0):
		...
		self.encoder = EncoderATT(self.lang.n_words, hidden_size, self.dropout)
	...
	def encode_and_decode(self, data, use_teacher_forcing, slot_temp):
		...
		#encoded_outputs= self.encoder(story)
		...
		# all_point_outputs, all_gate_outputs, words_point_out, words_class_out = self.decoder.forward(batch_size, \
		# 	encoded_outputs, data['context_len'], story, max_res_len, data['generate_y'], \
		# 	use_teacher_forcing, slot_temp) 
...
class EncoderATT(nn.Module):
	def __init__(self, vocab_size, hidden_size, dropout, n_layers=1):
		...
		self.MH = nn.MultiheadAttention(hidden_size,1,dropout)       #  change here
	...
	def forward(self, input_seqs, input_lengths, hidden=None):
		...
		out, _ = self.MH(embedded,embedded,embedded)
		embedded += torch.sigmoid(out)
		embedded = embedded.transpose(0,1)
...
class GeneratorATT(nn.Module):
	def __init__(self, lang, shared_emb, vocab_size, hidden_size, dropout, slots, nb_gate):
		...
		self.slots = slots
		self.MH = MultiheadAttention(hidden_size,1)
	...
	def forward(self, batch_size,  encoded_outputs, encoded_lens, story, max_res_len, target_batches, use_teacher_forcing, slot_temp):
		...
		for slot in slot_temp:
			...
			decoder_input = torch.reshape(decoder_input,[batch_size,1,self.hidden_size])
			decoder_input,_ = self.MH(decoder_input)
			decoder_input = torch.reshape(decoder_input,[batch_size,self.hidden_size])
			for wi in range(max_res_len):
				dec_state = self.multi_head_attention(torch.reshape(decoder_input,[batch_size,1,self.hidden_size]),encoded_outputs,encoded_outputs,1)
				context_vec, logits, prob = self.attend(encoded_outputs, dec_state.squeeze(1), encoded_lens)
				if wi == 0: 
					all_gate_outputs[counter] = self.W_gate(context_vec)
				p_vocab = self.attend_vocab(self.embedding.weight, dec_state.squeeze(1))
		...
		if args["parallel_decode"]:
			...
			for slot in slot_temp:
				...
				for wi in range(max_res_len):
					...
					if wi == 0: 
						all_gate_outputs[counter] = self.W_gate(context_vec)
					p_vocab = self.attend_vocab(self.embedding.weight, dec_state.squeeze(0))
					p_gen_vec = torch.cat([dec_state.squeeze(0), context_vec, decoder_input], -1)
					vocab_pointer_switches = self.sigmoid(self.W_ratio(p_gen_vec))
					p_context_ptr = torch.zeros(p_vocab.size())
					if USE_CUDA: p_context_ptr = p_context_ptr.cuda()
